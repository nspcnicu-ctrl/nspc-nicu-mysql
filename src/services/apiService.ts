...
export function mapRowToPatient(row: any): Patient {
  ...
  return {
    ...
    admissionTime: row.admission_time || row.admissionTime || (safeJsonParse(row.initial_anthropometry, {}) as Record<string, any>)?.admissionTime || undefined,
    readyToDischargeDate: row.ready_to_discharge_date || row.readyToDischargeDate || (safeJsonParse(row.discharge_summary, {}) as Record<string, any>)?.readyToDischargeDate || undefined,
    readyToDischargeTime: row.ready_to_discharge_time || row.readyToDischargeTime || (safeJsonParse(row.discharge_summary, {}) as Record<string, any>)?.readyToDischargeTime || undefined,
    dischargeDate: row.discharge_date || row.dischargeDate || (safeJsonParse(row.discharge_summary, {}) as Record<string, any>)?.dischargeDate || undefined,
    dischargeTime: row.discharge_time || row.dischargeTime || (safeJsonParse(row.discharge_summary, {}) as Record<string, any>)?.dischargeTime || undefined,
