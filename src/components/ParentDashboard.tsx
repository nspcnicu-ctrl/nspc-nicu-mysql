...
export const ParentDashboard: React.FC<ParentDashboardProps> = ({
  ...
  const currentLog = patient.dailyLogs[selectedLogIndex] || patient.dailyLogs[0];
  ...
  const chartData = [...patient.dailyLogs]
    .reverse()
    .map((log) => ({
      dateLabel: formatShortDate(log.date),
      fullDate: formatIndonesianDate(log.date),
      weight: log.weightGram,
      periodLabel: log.periodLabel,
      notes: log.nakesNotes,
    }));
              ...
              return (
                ...
                >
                  ...
                  <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden flex items-center justify-center">
                    ...
                    {/* Top Left Badge "NEW" (Bright Green Pill) */}
                  ...
                  <div className="p-4 space-y-2.5 flex-1 flex flex-col justify-between bg-white">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                      <button
                        type="button"
                        onClick={() => setPreviewPdfItem(pdf)}
                        className="py-2 px-2 bg-slate-50 hover:bg-teal-50 text-teal-800 border border-slate-200 font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => handleDownloadPdf(pdf)}
                        className="py-2 px-2 bg-teal-800 hover:bg-teal-900 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs"
                      >
          ...
          <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 z-[9999] animate-fadeIn font-sans">
            <div className="bg-white rounded-[28px] max-w-3xl w-full flex flex-col shadow-2xl overflow-hidden relative max-h-[94vh] border border-slate-100">
              ...
              <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1 bg-white">
                ...
                {/* GAMBAR POSTER / MODUL EDUKASI CONTAINER */}
                <div className="w-full bg-[#1e293b] rounded-3xl p-4 sm:p-6 flex items-center justify-center shadow-inner overflow-hidden min-h-[380px] border border-slate-800">
                  {(() => {
                    const displayImg =
                      renderedCovers[previewPdfItem.id] ||
                    ...
                    return (
                      <img
                        ...
                        className="max-h-[580px] w-auto max-w-full object-contain rounded-2xl shadow-2xl transition-transform duration-300 hover:scale-[1.01] bg-white"
                      />
                    );
                  })()}
                </div>
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
          ...
          {patient.milestones.bolehPulang && (
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {MILESTONE_DEFINITIONS.map((def) => {
            const isAchieved = Boolean(patient.milestones[def.key as keyof typeof patient.milestones]);
